...
class OptimizationMode(Enum):
    ...
    LocalDominanceByCov = "LocalDominanceByCov"
    LocalDominanceByCost = "LocalDominanceByCost"
...
class RRTInspectionPlanner(object):
    ...
    def plan(self, verbose=True, return_logs=False, optimization_mode: OptimizationMode = OptimizationMode.NoOptimization):
        ...
        while True:
            ...
            if self.planning_env.edge_validity_checker(near_state, new_state):
                ...
                if optimization_mode == OptimizationMode.LocalDominanceByCov or optimization_mode == OptimizationMode.LocalDominanceByCost:
    ...
    def local_dominated(self, new_state, near_states):
        ...
        near_states = [state for state in near_states if self.tree.get_total_inspected_points(state).union(self.tree.get_inspected_points(new_state)) > (self.tree.get_total_inspected_points(new_state))]
